function consultarContratoAnalise(instanceId) {
    ...
    var dsContratoAnalise = DatasetFactory.getDataset("dsTratativasContratoFinanceiro", null, constraints2, null);
    //adicionando o body da tabela
    document.getElementById('tableBody_'+instanceId).innerHTML = ''
    for (let index = 0; index < dsContratoAnalise.values.length; index++) {
        //criando o filtro para pegar o processo atual
        var constraints = new Array()
        constraints.push(DatasetFactory.createConstraint("cardDocumentId", dsContratoAnalise.values[index].documentid, dsContratoAnalise.values[index].documentid, ConstraintType.MUST));
        ...
        if(workflowProcess.values.length > 0){
            ...
            var body = '<tr>'
            body += '<td>'+dsContratoAnalise.values[index].codigoCliente+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].nomeCliente+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].codigoFilial+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].empreendimento+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].lote+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].quadra+'</td>'
            body += '<td>'+dsContratoAnalise.values[index].situacao+'</td>'
